...
/**
 * Seeds the "voice-templates" permission catalogue (create/read/update/
 * delete) and grants read+create+delete to "member" and everything to
 * "super-admin". Idempotent - safe to run against a DB that already has
 * some of these rows.
 */
...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const existingPermissions = await queryInterface.sequelize.query(
      `SELECT id, slug FROM permissions WHERE module = 'voice-templates'`,
      { type: Sequelize.QueryTypes.SELECT },
    );
    const existingSlugs = new Set(existingPermissions.map((p) => p.slug));

    const newPermissionRows = ACTIONS.filter((action) => !existingSlugs.has(`voice-templates.${action}`)).map(
      (action) => ({
        id: uuidv4(),
        name: `${titleCase(action)} Voice Templates`,
        slug: `voice-templates.${action}`,
        module: 'voice-templates',
        description: null,
        createdAt: now,
        updatedAt: now,
        deletedAt: null,
    ...
    for (const role of [superAdminRole, memberRole].filter(Boolean)) {
      ...
      const relevantPermissions = allPermissions.filter((p) =>
        isMember ? MEMBER_ACTIONS.some((action) => p.slug === `voice-templates.${action}`) : true,
      );
  ...
  async down(queryInterface, Sequelize) {
    const permissions = await queryInterface.sequelize.query(
      `SELECT id FROM permissions WHERE module = 'voice-templates'`,
      { type: Sequelize.QueryTypes.SELECT },
    );
